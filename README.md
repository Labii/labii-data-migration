...
### migrate_onenote_as_entries.py
`migrate_onenote_as_entries.py` is a command-line utility that helps you migrate your notes from Microsoft OneNote into Labii as structured entries. It connects to OneNote through the Microsoft Graph API, retrieves your notebooks, sections, and pages, then transforms each page’s content into either HTML or Markdown before creating corresponding entries in Labii via its REST API. The script also supports optional migration of embedded files and images as Labii attachments, provides a --dry-run mode for safe testing, and maintains a migration log to avoid duplications—making it a practical tool for smoothly moving your research notes and documentation into Labii.